...
func NewS3WatStore() (WatStore, error) {
	...
	config := filestore.S3FSConfig{
		S3Id:     os.Getenv(fmt.Sprintf("%s_%s", watProfile, awsAccessKeyId)),
		S3Key:    os.Getenv(fmt.Sprintf("%s_%s", watProfile, awsSecretAccessKey)),
		S3Region: os.Getenv(fmt.Sprintf("%s_%s", watProfile, awsDefaultRegion)),
		S3Bucket: os.Getenv(fmt.Sprintf("%s_%s", watProfile, awsS3Bucket)),
	}
...
// NewWatStore produces a WatStore backed by an S3 bucket
// if no arguments are supplied, the manifestid will get loaded from the environment
// @TODO: Switch to aws golang v2 s3 api and use profile for connection?????
// @TODO: make sure file operations use io and readers and stream chunks.  avoid large files in memory.
func NewWatStore(manifestArgs ...string) (WatStore, error) {
...
func (ws *S3WatStore) SetPayload(p Payload) error {
	...
	_, shouldFormat := os.LookupEnv("WAT_PAYLOAD_FORMATTED")
	var data []byte
	var err error
	if shouldFormat {
